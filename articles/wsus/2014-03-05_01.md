...
> [!NOTE]
> SQL Server Management Studio のバージョンによっては、インスタンス接続時に表示されるネットワーク接続の暗号化オプションが既定で「必須」となっているため、既定値のままだと接続に失敗します。WID はネットワークを経由する接続には対応していないため、WID に接続する場合には、暗号化オプションでは「オプション」を選択して接続してください。
> SQL Server をご利用の場合は、接続の暗号化用の証明書をお客様がご用意しているか次第となりますが、WID と同じように暗号化オプションで「オプション」を選択するか、「サーバー証明書を信頼する」オプションを有効化してご接続ください。
...
## 改訂履歴
2026-08-20: SQL Server Management Studio 接続時の注意事項を追記
2014-03-05: 初版