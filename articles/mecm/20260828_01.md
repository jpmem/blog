---
title: CMG 経由でクライアントをインストールする際の CCMSetup.exe の動作
date: 2026-08-28
categories:
...
- [CCMSetup.exe 実行時の Configuration Manager クライアント インストールの流れ](https://jpmem.github.io/blog/mecm/20260826_01/)
...
ただし、後述の検証 3 のとおり、CMG の管理ポイントを使用できるかどうかは、指定したパスの形式ではなく、クライアントがいずれかの方法で認証できるかどうかで決まります。

## 検証 1: 認証手段がない場合に CMG が候補から外れる
...
## 検証 2: Microsoft Entra ID による認証を指定した場合

次に、パスの [CCM_Proxy_MutualAuth] を [CCM_Proxy_ServerAuth] に置き換え、Microsoft Entra ID 関連のプロパティを追加して実行しました。イントラネットの管理ポイントへ到達できない状態は検証 1 と同じです。
...
### Microsoft Entra ID のオンボード情報の保存
...
### CMG の管理ポイントが候補に採用される

検証 1 と同様にクライアント認証証明書は見つかりませんでしたが、今回は Microsoft Entra ID による認証が使えるため、[Ignoring this MP] は記録されず、CMG の管理ポイントが [Source List] の先頭に採用されています。
...
### 管理ポイントの選択と Microsoft Entra ID のトークン取得

[CCMSetup.exe] は候補を順に確認し、応答が得られた管理ポイントを使用します。この検証では [Source List] の先頭にある CMG の URL が確認され、Microsoft Entra ID のデバイス トークンを取得したうえで、CMG 上の [CCM_STS] からクライアント認証用のトークンを取得しました。
...
## 検証 3: クライアントに Entra ID の情報が保存されている場合

検証 2 の後、同じクライアントに対して、Microsoft Entra ID 関連のプロパティを付けずに [CCM_Proxy_MutualAuth] のパスを指定して実行しました。検証 1 と同じコマンド ラインですが、結果は異なります。
