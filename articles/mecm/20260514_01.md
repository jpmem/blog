...
なお、オフライン アップデート手順やアップデートの前提条件などの一般的な情報につきましては下記ブログにてご紹介しておりますので併せてご参考いただけますと幸いでございます。  
...
1. インターネットへ接続可能な端末で、下記公開情報の「バージョン 2603 オプトイン スクリプト」をクリックし、EnableEarlyUpdateRing2603.exe をダウンロードします  
   ...
   Title: 早期更新リング  
   URL: https://learn.microsoft.com/ja-jp/intune/configmgr/core/servers/manage/checklist-for-installing-updates  
...
4. 下記コマンドを実行し、UsageData.cab を生成します  
   ```  
   .\serviceconnectiontool.exe -prepare -usagedatadest C:\<保存先パス>\UsageData.cab  
   ```  
   ※ <保存先パス> は cab ファイルの保存先を任意で設定ください  
...
6. 下記コマンドを実行し、更新データをダウンロードします  
   ```  
   .\serviceconnectiontool.exe -connect -downloadsiteversion -usagedatasrc C:\<cab ファイルの保存先パス> -updatepackdest C:\<更新データのダウンロード先パス>  
   ```  
   ※ 「A. cab ファイルの準備」で生成された cab ファイルを `-usagedatasrc` のソース ファイルとして指定し、更新データの保存先パスを `-updatepackdest` のダウンロード パスとして指定します  
...
5. 下記コマンドを実行し、更新データをインポートします  
   ```  
   .\serviceconnectiontool.exe -import -updatepacksrc C:\<更新データの保存先パス>  
   ```  
   ※ 「B. 更新データのダウンロード」で生成された更新データの保存先パスを、`-updatepacksrc` のソースファイルとして指定します。  
