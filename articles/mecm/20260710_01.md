...
不要判定の軸を含めた要件が決まったら、スクリプトを作成していきます。
...
以下、条件例ごとのサンプル スクリプトとなります。

1. x86, ARM64 アーキテクチャの更新プログラムは不要とする (x64 のみ配る)
2. 1 年以上前にリリースされた更新プログラムはすべて不要とする
3. 置き換えられた更新プログラムのうち、リリースされてから 3 ヵ月以上経過した更新プログラムは不要とする
4. Edge は Beta, Dev, Stable チャネルは不要とする。 (Extended Stable チャネルのみ配る)
5.  Microsoft 365 Apps は月次、カレントチャネルは不要とする。 (半期チャネルのみ配る)

### サンプル 1: x86, ARM64 アーキテクチャの更新プログラムは不要とする (x64 のみ配る)

※ 本サンプルは全製品、全分類に適用されるので、もし対応が必要な場合は $targetScope.Categories とClassifications にて設定ください。
...
### サンプル 2: 1 年以上前にリリースされた更新プログラムはすべて不要とする
...
### サンプル 3: 置き換えられた更新プログラムのうち、リリースされてから 3 ヵ月以上経過した更新プログラムは不要とする
...
### サンプル 4: Edge は Beta, Dev, Stable チャネルは不要とする。 (Extended Stable チャネルのみ配る)
...
### サンプル 5: Microsoft 365 Apps は月次、カレントチャネルは不要とする。 (半期チャネルのみ配る)
...
$excludedM365Pattern = '^Microsoft 365 Apps Update - (?<channel>(?!Semi-Annual)(?:Current Channel(?: \(Preview\))?|Monthly Enterprise Channel)) (?<updateType>Extended Quality Update|Feature Update|Quality Update) for (?<arch>ARM64|x64|x86) based Edition Version (?<version>\d{4}) \(Build (?<build>\d+\.\d+)\)$'
