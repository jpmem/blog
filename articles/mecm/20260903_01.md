---
title: CCMSetup.exe によるクライアント インストールについての動作説明
date: 2026-09-03
categories:
...
2. 管理ポイントへ問い合わせ、クライアント インストール コンテンツを持つ配布ポイントを特定し、マニフェストを確認する
3. マニフェストに基づき、前提コンポーネントとクライアント ファイルの要否を判定する
4. 必要なファイルを Background Intelligent Transfer Service (BITS) でダウンロードし、ハッシュと信頼性を検証する
5. [client.msi] を起動する
6. クライアントのファイル、レジストリ、コンポーネント、サービスなどを構成する
7. 言語パックなどを構成し、[CCMSetup.exe] が戻り値 0 で終了する
